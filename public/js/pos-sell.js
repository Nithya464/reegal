$(document).ready(function() {
        ...
        $('#search_product')
            .autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    if (1 == 1 || ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || for_so || is_draft) {
                        $(this).val(null);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
        __write_number(tr.find('input.pos_unit_price_inc_tax'), line_total, false, 2);
        ...
        var line_total = unit_price_inc_tax;
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function() {
        ...
        __write_number(tr.find('input.pos_unit_price_inc_tax'), line_total);
    ...
    $('table#pos_table').on('change', 'select.sub_unit', function() {
        ...
        var base_unit_selling_price = selected_option.data('base-price')
        
        // var multiplier = parseFloat(selected_option.data('multiplier'));
        var multiplier = parseFloat(tr.find('.base_unit_multiplier').val());
        ...
        var current_stock = parseInt(tr.find('input.sub_total_quantity').val() / selected_option.data('multiplier'));
        ...
        // if (base_max_avlbl) {
            ...
            var max_err_msg = __translate(error_msg_line, {
                ...
                unit_name: selected_option.data('name'),
            });
            
            // qty_element.attr('data-rule-max-value', max_avlbl);
            qty_element.attr('data-rule-max-value', current_stock);
            qty_element.attr('data-msg-max-value', max_err_msg);
            qty_element.rules('add', {
                // 'max-value': max_avlbl,
                'max-value': current_stock,
                messages: {
                    'max-value': max_err_msg,
                },
            });
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            method: 'GET',
            url: '/pos-sells/get_product_row/' + variation_id + '/' + location_id,
            async: false,
            data: {
...
function pos_each_row(row_obj) {
    ...
    __write_number(row_obj.find('input.pos_unit_price_inc_tax'), line_total);
...
jQuery('body').on('change','input#main_shipping_charges',function(){
    pos_total_row();
});

jQuery('body').on('change', '.main_overall_discount_type, .main_overall_discount', function() {
    pos_total_row();
});
...
function pos_total_row() {
    ...
    $('table#pos_table tbody tr').each(function() {
        ...
        //total_quantity = total_quantity + __read_number($('input.pos_quantity'));
    });
    ...
    var main_shipping_charges = __read_number($('input#main_shipping_charges'));
    var price_total = price_total + main_shipping_charges;

    var discount_type = $('.main_overall_discount_type').val();
    var discount_amount = __read_number($('input.main_overall_discount'));
    if (discount_amount) {
        if (discount_type == 'fixed') {
            price_total = price_total - discount_amount;
        } else {
            price_total = __substract_percent(price_total, discount_amount);
        }
    }

    __write_number($('input#main_sub_total'), price_total);
    __write_number($('input#main_order_total'), Math.round(price_total));
    __write_number($('input#main_payable_amount'), Math.round(price_total));
    var price_total = (Math.round(price_total) - price_total);
    __write_number($('input#main_adjustment'), price_total);
    

    var product_weight = 0;
    var product_ml_qty = 0;
    $('table#pos_table tbody tr').each(function() {
        product_weight = (product_weight + __read_number($('input.product_weight'))) * __read_number($('input.input_quantity'));
        product_ml_qty = (product_ml_qty + __read_number($('input.product_ml_qty'))) * __read_number($('input.input_quantity'));
    });

    __write_number($('input#main_weight_quantity'), product_weight);
    __write_number($('input#main_ml_quantity'), product_ml_qty);
    // $('#main_weight_quantity').val(product_weight);
    // $('#main_ml_quantity').val(product_ml_qty);
...
function calculate_discounted_unit_price(row) {
    var this_unit_price = __read_number(row.find('input.pos_unit_price_inc_tax'));
...
function update_shipping_address(data) {
    if ($('#shipping_address_div').length) {
        ...
        if (data.shipping_address_1) {
            address.push('<br>' + data.shipping_address_1);
        }
        if (data.shipping_address_2) {
            address.push('<br>' + data.shipping_address_2);
        }
        if (data.shipping_city) {
            address.push(data.shipping_city);
        }
        if (data.shipping_state) {
            address.push(data.shipping_state);
        }
        if (data.country) {
            address.push(data.country);
        }
        if (data.shipping_zipcode) {
            address.push('<br>' + data.shipping_zipcode);
        }
    ...
    if($("#sales_person_name").length){
        $("#sales_person_name").val(data.user_name);
        $("#sales_person_id").val(data.user_id);
    }
}
