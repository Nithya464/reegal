$(document).ready(function() {
    ...
    $(document).on('keyup', '.purchase_quantity', function() {
        ...
        var unit_quantity = quantity * multiplier;

        var sp_element = row.find('input.sub_total_quantity');
        __write_number(sp_element, unit_quantity);

        row.find('span.sub_total_quantity').html(unit_quantity);

        // var row_subtotal_after_tax_hidden = (unit_quantity / multiplier) * base_price;
        // __write_number(row.find('input.row_subtotal_after_tax_hidden'), row_subtotal_after_tax_hidden, true);
        
        var purchase_product_price = row.find('input.purchase_product_price').val();
        var purchase_product_total_price = purchase_product_price * quantity;
        row.find('input.purchase_product_total_price').val(purchase_product_total_price);
    ...
    //On Change of quantity
    $(document).on('keyup', '.purchase_product_price', function() {
        var row = $(this).closest('tr');

        var purchase_product_price = __read_number($(this), true);
        var purchase_quantity = row.find('input.purchase_quantity').val();
        
        var purchase_product_total_price = purchase_product_price * purchase_quantity;
        row.find('input.purchase_product_total_price').val(purchase_product_total_price);
    ...
    $('table#purchase_entry_table').on('change', 'select.sub_unit', function() {
        ...
        var purchase_quantity = tr.find('input.purchase_quantity').val();
        ...
        tr.find('input.purchase_product_price').val(base_price);
       // var unit_sp = base_unit_selling_price * multiplier;
        //var unit_cost = base_unit_cost * multiplier;
        var unit_quantity = purchase_quantity * multiplier;

        // var sp_element = tr.find('input.default_sell_price');
        // __write_number(sp_element, unit_sp);

        var sp_element = tr.find('input.sub_total_quantity');
        __write_number(sp_element, unit_quantity);

        tr.find('span.sub_total_quantity').html(unit_quantity);

        // var row_subtotal_after_tax_hidden = (unit_quantity / multiplier) * base_price;
        // __write_number(row.find('input.row_subtotal_after_tax_hidden'), row_subtotal_after_tax_hidden, true);

        var purchase_product_price = tr.find('input.purchase_product_price').val();
        var purchase_product_total_price = purchase_product_price * purchase_quantity;
        tr.find('input.purchase_product_total_price').val(purchase_product_total_price);
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        var show_price = $("#show_price").length;

        if($("#productId"+product_id).length > 0){
            var row = $("#productId"+product_id).closest('tr');
            var purchase_quantity = parseInt(parseInt(row.find('input.purchase_quantity').val()) + 1);
            row.find('input.purchase_quantity').val(purchase_quantity);
...
function update_purchase_entry_row_values(row) {
    if (typeof row != 'undefined') {
       ...
       // var row_subtotal_after_tax = quantity * unit_cost_price_after_tax;
        ...
        var row_subtotal_after_tax = quantity * base_price;
...
function update_table_total() {
    ...
    $('#purchase_entry_table tbody')
        ...
        .each(function() {
            total_quantity += __read_number($(this).find('.purchase_quantity'), true);
            total_subtotal += __read_number($(this).find('.purchase_product_total_price'), true);
        });
    $('#total_quantity').text(__number_f(total_quantity, false));
    __write_number($('input#total_subtotal_input'), total_subtotal, true);
    $('#total_subtotal').text(__currency_trans_from_en(total_subtotal, true, true));
...
$(document).on('click', 'button.submit_purchase_IR_form', function(e) {
    e.preventDefault();

    if(typeof($(this).data('is-confirm')) != "undefined"){
        swal({
            title: LANG.sure,
            text: 'You want to '+$(this).data('msg')+' this P.O ?',
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
...
jQuery('body').on('change','#to_location_id',function(){
    toggle_search();
});
function toggle_search() {
    if (($('#supplier_id').val() && $('#to_location_id').val()) || $('#supplier_id').val() || $('#to_location_id').val() || $('#location_id').val()) {
