$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    ...
                    var evt = new CustomEvent("brandAdded", {
                        detail: result.data
                    });
    ...
    }else if(contact_table_type == 'draft_customer'){
        var columns = [
            { data: 'action', searchable: false, orderable: false },
            { data: 'name', name: 'name' },
            ...
            { data: 'bussiness_name', name: 'bussiness_name' },
            { data: 'otp_licence', name: 'otp_licence' },
            { data: 'sales_name', name: 'sales_name' },
            { data: 'term', name: 'term' },
            { data: 'shipping_address_1', name: 'shipping_address_1' },
    ...
    $('.routes_modal').on('shown.bs.modal', function (e) {
        $(".routes_modal").find('.select2').each(function () {
            $(this).select2();
        });
    });
    $(document).on('submit', 'form#route_add_form', function (e) {
        e.preventDefault();
        var form = $(this);
        var data = form.serialize();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    $('div.routes_modal').modal('hide');
                    toastr.success(result.msg);
                    if (typeof routes_table !== 'undefined') {
                        routes_table.ajax.reload();
                    }
                    var evt = new CustomEvent("routeAdded", {
                        detail: result.data
                    });
                    window.dispatchEvent(evt);
                    //event can be listened as
                    //window.addEventListener("routeAdded", function(evt) {}
    ...
    //routes table
    var routes_table = $('#routes_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/routes',
        columnDefs: [{
    ...
    $(document).on('click', 'button.edit_route_button', function () {
        $('div.routes_modal').load($(this).data('href'), function () {
            $(this).modal('show');

            $('form#route_edit_form').submit(function (e) {
                e.preventDefault();
                var form = $(this);
                var data = form.serialize();
    ...
    $(document).on('click', 'button.delete_route_button', function () {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_route,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
    ...
    /////////////////////// Price Levels ///////////////////////////////
    $(document).on('submit', 'form#price_level_add_form', function (e) {
        e.preventDefault();
        var form = $(this);
        var data = form.serialize();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    $('div.price_levels_modal').modal('hide');
                    toastr.success(result.msg);
                    if (typeof price_levels_table !== 'undefined') {
                        price_levels_table.ajax.reload();
                    }
                    var evt = new CustomEvent("price_levelAdded", {
                        detail: result.data
                    });
                    window.dispatchEvent(evt);
                    //event can be listened as
                    //window.addEventListener("price_levelAdded", function(evt) {}
    ...
    //price_levels table
    var price_levels_table = $('#price_levels_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/price_levels',
        columnDefs: [{
    ...
    $(document).on('click', 'button.edit_price_level_button', function () {
        $('div.price_levels_modal').load($(this).data('href'), function () {
            $(this).modal('show');

            $('form#price_level_edit_form').submit(function (e) {
                e.preventDefault();
                var form = $(this);
                var data = form.serialize();
    ...
    $(document).on('click', 'button.delete_price_level_button', function () {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_price_level,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
...
//Start: CRUD for unit
//Unit table
var units_table = $('#car_table').DataTable({
    processing: true,
    serverSide: true,
    ajax: '/cars',
    columnDefs: [{
    ...
    columns: [{
        ...
        {
            data: 'load_order_type',
            name: 'load_order_type'
        },
        {
            data: 'status',
            name: 'status'
