...
class PurchaseReturnController extends Controller
{
            ...
            return Datatables::of($purchases_returns)
                ->addColumn('action', function ($row) {
                    ...
                    if ($row->payment_status != 'paid') {
                        $html .= '<li><a href="'.action([\App\Http\Controllers\TransactionPaymentController::class, 'addPayment'], [$row->id]).'" class="add_payment_modal"><i class="fas fa-money-bill-alt"></i>'.__('purchase_return.add_payment').'</a></li>';
                    }

                    $html .= '<li><a href="'.action([\App\Http\Controllers\TransactionPaymentController::class, 'show'], [$row->id]).'" class="view_payment_modal"><i class="fas fa-money-bill-alt"></i>'.__('purchase_return.view_payments').'</a></li>';
                ...
                ->editColumn('parent_purchase', function ($row) {
                    ...
                    if (! empty($row->parent_purchase)) {
                        $html = '<a href="#" data-href="'.action([\App\Http\Controllers\PurchaseController::class, 'show'], [$row->return_parent_id]).'" class="btn-modal" data-container=".view_modal">'.$row->parent_purchase_return.'</a>';
                    }
    ...
    {
        if (! auth()->user()->can('purchase_return.delete')) {
            abort(403, 'Unauthorized action.');
        }
