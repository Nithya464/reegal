...
<section class="content no-print">
    ...
    <form class="form-group mt-10" action="{{ route('receivePayment_update') }}" method="POST" id="receive_payment_edit" name="receive_payment_edit">
        @csrf
        @include('pos-payment.partials.payment_list_edit')
    </form>
        {{-- @if(!empty($sources))
            <div class="col-md-3">
...
<div class="modal fade edit_payment_modal" tabindex="-1" role="dialog" 
  ...
  <!-- Modal -->
    <!-- Modal -->
    <div class="modal fade due_payment" id="exampleModal111" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    ...
                    if(response.data.length > 0) {
                        ...
                        $.each(response.data, function(key, val) {
                            ...
                            html += '<tr>' +
                                '<td>' +
                                '<div class="form-group">' +
                                '<input type="checkbox" class="form-check-input" name="selected_ids[]"  checked>' +
                                '</div>' +
                                '</td>' +
                                '<td>' + val.ref_no + '</td>' +
