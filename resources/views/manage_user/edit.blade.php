...
<section class="content">
    ...
    <div class="row">
        ...
        @component('components.widget', ['title' => __('lang_v1.roles_and_permissions')])
            ...
            <div class="col-md-6">
              <div class="form-group">
                @php $location_id=0 @endphp
                @foreach($locations as $location)
                  @php 
                    $location_id=$location->id;
                  @endphp
                @endforeach
                  {!! Form::label('location_permissions', __('role.access_locations') . ':*') !!}     
                  <select name="location_permissions[]" class="form-control required" id="location_permissions">
...
<script type="text/javascript">
  $(document).ready(function(){
   ...
   var location_id = '<?php echo json_encode($permitted_locations_ids);?>'
   $('#location_permissions').val('location.'+location_id).trigger('change')
  ...
  $('form#user_edit_form').validate({
                rules: {
                    ...
                    email: {
                        email: true,
                        remote: {
                            url: "/business/registers/check-email",
                            type: "post",
                            data: {
                                email: function() {
                                    return $( "#email" ).val();
