@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            <input type="hidden" id="productId{{ $product->id }}">
            @php
                $check_decimal = 'false';
            ...
            <select name="purchases[{{$row_count}}][sub_unit_id]" data-count="{{ $row_count }}" class="unitIdClass form-control input-sm sub_unit">
                @foreach($product->product_unit_price as $key => $value)
                    @if($key == 0)
                        @php $poPieces = $value->qty * $quantity_value; @endphp
                    @endif
                    <option value="{{$value->id}}" data-multiplier="{{$value->qty}}" data-base-price="{{ $value->base_price }}">
        ...
        <td>
            ...
            @endif --}}
            {{ (($product->product_default_unit->unit)) ?? null }} - {{ (($product->product_stock->current_stock)) ?? 0; }}
        </td>
        {{-- <td>
        ...
        <td>
            <input type="text" 
                ...
                @endif
            >
            {{-- <input type="hidden" name="purchases[{{$row_count}}][sub_total_price]" class="purchase_product_price" value="{{ ($quantity_value * (($product->product_unit_price[0]->base_price)) ?? 0) }}"> --}}
        </td>
        ...
        @if($show_price)
            <td><input type="text" name="purchases[{{$row_count}}][price]" value="{{ $product->product_unit_price[0]->base_price }}" class="form-control input-sm purchase_product_price input_number mousetrap" required></td>
            <td><input type="text" name="purchases[{{$row_count}}][total_price]" value="{{ ($quantity_value * (($product->product_unit_price[0]->base_price)) ?? 0) }}" class="form-control input-sm purchase_product_total_price input_number mousetrap" readonly></td>
        @endif
