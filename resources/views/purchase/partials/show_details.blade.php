...
<div class="modal-body">
  ...
  <div class="row invoice-info">
    ...
    <div class="@if($purchase->to_location) col-sm-4 @else col-sm-6 @endif invoice-col">
      {{ __('Location') }}:
      <address>
    ...
    @if($purchase->to_location)
    <div class="col-sm-4 invoice-col">
      {{ __('To') }}:
      <address>
        <strong>{{ $purchase->business->name }}</strong>
        {{ $purchase->to_location->name }}
        @if(!empty($purchase->to_location->landmark))
          <br>{{$purchase->to_location->landmark}}
        @endif
        @if(!empty($purchase->to_location->city) || !empty($purchase->to_location->state) || !empty($purchase->to_location->country))
          <br>{{implode(',', array_filter([$purchase->to_location->city, $purchase->to_location->state, $purchase->to_location->country]))}}
        @endif
        ...
        @if(!empty($purchase->to_location->mobile))
          <br>@lang('contact.mobile'): {{$purchase->to_location->mobile}}
        @endif
        @if(!empty($purchase->to_location->email))
          <br>@lang('business.email'): {{$purchase->to_location->email}}
        @endif
      </address>
    </div>
    @endif
    ...
    <div class="@if($purchase->to_location) col-sm-4 @else col-sm-6 @endif invoice-col">
      ...
      <b>@lang('purchase.po_no'):</b> #{{ $purchase->po_no }}<br/>
        ...
        <table class="table bg-gray">
          <thead>
            <tr class="bg-green">
              ...
              <th class="text-center" colspan="3">PO</th>
          ...
          @foreach($purchase->purchase_lines as $purchase_line)
            <tr>
              ...
              <td>{{ $purchase_line->product->id }}</td>
              <td>
              ...
              <td>{{ $purchase_line->product_unit_price->unit }}({{ $purchase_line->product_unit_price->qty }})</td>
              <td>{{ ($purchase_line->quantity) }}</td>
              <td>{{ $purchase_line->secondary_unit_quantity }}</td>
              @if(isset($show_price) && $show_price)
              <td>{{ ($purchase_line->purchase_price) }}</td>
              <td>{{ ($purchase_line->purchase_price_total) }}</td>    
              @endif
              {{-- <td>
                 @if( $purchase_line->product->type == 'variable')
  ...
  <div class="row">
    ...
    @endif --}}
    <div class="col-md-12 col-sm-12 col-xs-12 @if($purchase->type == 'purchase_order') col-md-offset-10 @endif">
      <div class="table-responsive">
        <table class="table">
          <!-- <tr class="hide">
          ...
          <tr class="pull-right">
            ...
            <td></td>
            <td><span class="display_currency pull-right">{{ count($purchase->purchase_lines) }}</span></td>
          </tr>
          {{-- <tr>
