  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <div class="form-group">
        ...
        {!! Form::text('website_sequence_no', $category->website_sequence_no, ['class' => 'form-control input_number', 'placeholder' => __( 'Enter Website Sequence No')]); !!}
      </div>
      <div class="form-group">
        {!! Form::label('status', __( 'Status' ) . ':') !!}
          {!! Form::select('status', config('global.status_dropdown'), $category->status, ['class' => 'form-control']); !!}
      </div>
      <div class="form-group">
        {!! Form::label('show_on_website', __( 'Show On Website' ) . ':') !!}
        {!! Form::checkbox('show_on_website', 1, ($category->show_on_website==1) ? true : false,[ 'class' => 'toggler','unchecked_value' => 2]); !!} 
      </div>
      @if(!empty($parent_categories) && $enable_sub_category)
          <div class="form-group">
            <div class="checkbox">
              <label>
                 {!! Form::checkbox('add_as_sub_cat', 1, !$is_parent,[ 'class' => 'toggler', 'data-toggle_id' => 'parent_cat_div' ]); !!} Add as sub-category
              </label>
            </div>
          </div>
