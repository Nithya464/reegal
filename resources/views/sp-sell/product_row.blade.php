@php
	...
	$product_unit_price = App\ProductUnitPrice::where('product_id',$product->product_id)->get();
...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
    <input type="hidden" name="products[{{$row_count}}][weight]" value="{{ $product->weight }}" class="product_weight">
    <input type="hidden" name="products[{{$row_count}}][ml_qty]" value="{{ $product->ml_qty }}" class="product_ml_qty">
	<td>{!! $product->sku !!}</td>
	<td>
		@if(!empty($so_line))
			<input type="hidden" 
		...
		@php
			...
			$unit_price_inc_tax = $product->unit_price_before_discount;
	...
	<td>
		<select name="products[{{$row_count}}][product_sub_unit_id]" data-count="{{ $row_count }}" class="form-control input-sm sub_unit" readonly>
			@php
				$quantity_value = $product->quantity_ordered;
			@endphp
			@foreach($product_unit_price as $key => $value)
				@if($key == 0)
					@php $poPieces = $value->qty * $quantity_value; @endphp
				@endif
				<option value="{{$value->id}}" data-multiplier="{{$value->qty}}" data-name="{{ $value->unit }}" data-base-price="{{ $value->base_price }}">
	...
	<td>
		...
		<div class="input-group input-number">
			<span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-down" {{ $disabled }}><i class="fa fa-minus text-danger"></i></button></span>
		<input type="text" data-min="1" 
			class="form-control pos_quantity input_number mousetrap input_quantity" {{ $disabled }} 
			...
			@endif  --}}
			@php
				$max_qty_rule_new = $product->current_stock;
				$max_qty_msg = __('lang_v1.quantity_error_msg_in_lot', ['qty'=> $max_qty_rule_new, 'unit' => 'Pieces'  ]);
			@endphp
			data-rule-max-value="{{$max_qty_rule_new}}" data-qty_available="{{$max_qty_rule_new}}" data-msg-max-value="{{$max_qty_msg}}">
		<span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-up" {{ $disabled }}><i class="fa fa-plus text-success"></i></button></span>
		</div>
		...
		<input type="hidden" class="base_unit_multiplier" name="products[{{$row_count}}][base_unit_multiplier]" value="{{$quantity_value}}">

		<input type="hidden" class="hidden_base_unit_sell_price" value="{{(($product_unit_price[0]->base_price)) ?? 0}}">
	...
	<td>
		{!! Form::hidden('products[' . $row_count . '][sub_total_quantity]', (($product->current_stock)) ?? 0, ['class' => 'sub_total_quantity']); !!}
		<span class="sub_total_quantity">{{ (($product->current_stock)) ?? 0; }}</span>
	</td>
	@if(!empty($is_direct_sell))
		...
		@php
			...
			$pos_unit_price = ($quantity_value * (($product_unit_price[0]->base_price)) ?? 0);
			if(!empty($so_line) && $action !== 'edit') {
				$pos_unit_price = $so_line->unit_price_before_discount;
			}
			$pos_unit_price = ($product->default_sell_price > 0) ? $product->default_sell_price : (($product_unit_price[0]->base_price)) ?? 0;
		@endphp
		{{-- <td class="@if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif"> --}}
		<td>
			<input type="text" name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{@num_format($pos_unit_price)}}" @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_unit_price}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_unit_price)])}}" @endif {{ $disabled }}> 
		...
		<td>
			{!! Form::text("products[$row_count][line_discount_amount]", @num_format($discount_amount), ['class' => 'form-control input_number row_discount_amount',$disabled]); !!}<br>
			{!! Form::select("products[$row_count][line_discount_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $discount_type , ['class' => 'form-control row_discount_type',$disabled]); !!}
			@if(!empty($discount))
	...
	<td class="text-center">
		@php
			...
			$pos_sell_price_inc_tax = ($product->sell_price_inc_tax > 0) ? $product->sell_price_inc_tax : (($product_unit_price[0]->base_price)) ?? 0;
		@endphp
		<input type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($pos_sell_price_inc_tax )}}">
		<span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency_symbol="true">{{$pos_sell_price_inc_tax}}</span>
	</td>
	@if(!$disabled)
