  ...
  <input type="hidden" id="purchase_id" value="{{ $purchase->id }}">
    ...
    @component('components.widget', ['class' => 'box-primary'])
        <div class="row">
            ...
            <div class="col-sm-4">
              <div class="form-group">
                {!! Form::label('po_no', __('PO No') . '*') !!}
                {!! Form::text('po_no', $purchase->po_no, ['class' => 'form-control', 'required','readonly']); !!}
              </div>
            </div>
            ...
            @php
              $isDisabled = ($isReceiver == true) ? 'disabled' : '';
            @endphp
            <div class="col-sm-4">
              <div class="form-group">
                {!! Form::label('location_id', __('purchase.business_location').':*') !!}
                @show_tooltip(__('tooltip.purchase_location'))
                {!! Form::select('to_location_id', $business_locations, $purchase->to_location_id, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), $isDisabled, 'id' => 'location_id']); !!}
              </div>
            </div>
            <div class="col-sm-4">
              <div class="form-group">
                {!! Form::label('status', __('purchase.status').':') !!}
                {!! Form::text('status', $purchase->status, ['class' => 'form-control', 'readonly']); !!}
              </div>
            </div>
    ...
    <div class="row">
        <div class="col-sm-12">
          ...
          @elseif($purchase->status == "pending")
           <button type="button" class="submit_purchase_IR_form btn btn-primary pull-right btn-flat" data-status="received">{{ __('Received P.O.') }}</button>
           @elseif($purchase->status == "received")
            <button type="button" class="submit_purchase_IR_form btn btn-primary pull-right btn-flat" data-status="verified">{{ __('Verified P.O.') }}</button>
          @endif
