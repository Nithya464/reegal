@extends('layouts.app')
@section('title', __('Generate PO'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>@lang('Generate PO') <i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="@include('purchase.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title="" title=""></i></h1>
</section>
...
<section class="content">
  ...
  {!! Form::open(['url' =>  action([\App\Http\Controllers\PurchaseOrderByVendorController::class, 'update'] , [$purchase->id] ), 'method' => 'PUT', 'id' => 'add_purchase_form', 'files' => true ]) !!}
  ...
  <input type="hidden" id="purchase_id" value="{{ $purchase->id }}">
    ...
    <div class="row">
        <div class="col-sm-12">
            <button type="button" class="submit_purchase_IR_form btn btn-primary pull-right btn-flat" data-status="draft">{{ __('Save As Draft') }}</button>
            <button type="button" class="submit_purchase_IR_form btn btn-success pull-right btn-flat mr-8" data-status="pending">{{ __('Submit PO') }}</button>
        </div>
    </div>
{!! Form::close() !!}
</section>
